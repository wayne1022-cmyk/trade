"""
ig_trader.py - IG Trading 交易執行模組（淨頭寸管理，只開倉不平倉）
目標淨頭寸：+0.2（多）或 -0.2（空），根據 AI 方向調整。
"""
...
class IGTrader:
    ...
    def place_order(self, signal: dict) -> dict | None:
        ...
        delta = target_net - current_net   # 需要調整的淨頭寸變化
        ...
        logger.info("目標淨頭寸：%.2f (%s)，目前淨頭寸：%.2f，需調整 %.2f 手 %s",
